...
public class DeleteExpenseByIdUseCase implements DeleteExpenseByIdBoundary {
    ...
    public void execute(String id, String ownerId) {
        if (!expenseRepository.existsById(id, ownerId)) {
            throw new ExpenseNotFoundException(id);
        }
        if (shareRepository.existsActiveBySourceId(ShareSourceType.EXPENSE, id, ownerId)) {
            throw new SourceInUseByShareException(ShareSourceType.EXPENSE, id);
        }

        rechargeableBullets(id);
        expenseRepository.deleteById(id, ownerId);
    }
