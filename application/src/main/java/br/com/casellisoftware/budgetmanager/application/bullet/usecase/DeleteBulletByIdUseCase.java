...
public class DeleteBulletByIdUseCase implements DeleteBulletByIdBoundary {
    ...
    public void execute(String id, String ownerId) {
        log.info("Deleting bullet id={} ownerId={}", id, ownerId);

        Bullet bullet = bulletRepository.findById(id, ownerId)
                .orElseThrow(() -> new BulletNotFoundException(id));

        if (paymentRepository.existsByBulletId(id, ownerId)) {
            throw new BulletInUseException(id);
        }

        Wallet wallet = findWalletDomainByIdBoundary.findById(bullet.getWalletId(), ownerId);
        Wallet refunded = wallet.credit(bullet.getRemaining());

        walletRepository.save(refunded);
        bulletRepository.deleteById(id, ownerId);
