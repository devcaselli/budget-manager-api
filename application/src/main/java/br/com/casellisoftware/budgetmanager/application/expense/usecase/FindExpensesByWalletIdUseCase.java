...
public class FindExpensesByWalletIdUseCase implements FindExpensesByWalletIdBoundary {
    ...
    @Override
    public PageResult<ExpenseOutput> execute(String walletId, int page, int size, boolean unhidden, String ownerId) {
        log.debug("Finding expenses for walletId={}, page={}, size={}, unhidden={}", walletId, page, size, unhidden);

        findWalletByIdBoundary.findById(walletId, ownerId);

        PageResult<Expense> expensePage = expenseRepository.findByWalletId(walletId, page, size, unhidden, ownerId);
