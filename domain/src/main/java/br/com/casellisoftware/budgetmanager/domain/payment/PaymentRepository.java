 ...
 *   <li>Scoped delete may throw {@link PaymentNotFoundException} when no row
 *       matches both id and ownerId.</li>
...
public interface PaymentRepository {
    ...
    default Optional<Payment> findById(String id, String ownerId) {
        return findById(id).filter(payment -> payment.getOwnerId().equals(ownerId));
    }

    PageResult<Payment> findByWalletId(String walletId, int page, int size);
    PageResult<Payment> findByWalletId(String walletId, int page, int size, String ownerId);

    void deleteById(String id, String ownerId);

    List<Payment> findAllByExpenseId(String expenseId);
    List<Payment> findAllByExpenseId(String expenseId, String ownerId);

    void deleteAllById(List<String> ids);

    boolean existsByBulletId(String bulletId);
    boolean existsByBulletId(String bulletId, String ownerId);
}
